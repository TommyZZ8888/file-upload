...
watch(
  ...
  (newVal) => {
    if (newVal) {
      selectedFormats.value = props.allowedFormats
        ? props.allowedFormats.split(",").map((f) => f.trim())
        : [];
      tempMaxSizeMB.value = props.maxUploadSize
        ? Math.round(props.maxUploadSize / (1024 * 1024))
        : 10;
